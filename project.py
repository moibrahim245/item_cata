...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'username' in login_session:
            ...
            flash('''Please login first for creating,
                  updating or deleting the restaurant''')
...
def fbconnect():
    ...
    # Get user picture
    url = '''https://graph.facebook.com/v2.8/me/picture?access_token=%s&redirect=
          0&height = 200&width = 200''' % token  # noqa
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
